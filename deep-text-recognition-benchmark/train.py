...
def train(opt):
    ...
    if opt.saved_model != '':
        ...
        if opt.FT:
            if isinstance(model, nn.DataParallel):
                ...
                loaded_state_dict = torch.load(opt.saved_model, map_location=device)
    ...
    if 'CTC' in opt.Prediction:
        ...
        criterion = torch.nn.CrossEntropyLoss(ignore_index=0, label_smoothing=0.2).to(device)  # ignore [GO] token = ignore index 0
    # loss averager
    loss_avg = Averager()
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', min_lr = 1e-6, factor=0.5)
    ...
    while(True):
        ...
        if (iteration == 28700):
            torch.save(
                model.state_dict(), f'./saved_models/{opt.exp_name}/iter_28700.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, default=448, help='input batch size')
